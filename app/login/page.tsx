/* ============================================
   ...
   - useAuth() for the sign-in methods
   - useRouter for redirecting after login
   
   Auth flow:
   1. User enters email/password OR clicks Google
   2. Firebase authenticates them
   3. onAuthStateChanged fires in AuthProvider
   4. AuthProvider updates user state
   5. We detect user is logged in → redirect to /validate
   
   Next.js metadata:
   Since this is a "use client" component, we can't export
   metadata directly. We'll use a separate layout or just
   rely on the parent layout's metadata.
   ============================================ */
...
export default function LoginPage() {
  ...
  /* If already logged in, redirect to the validate page.
     useEffect runs AFTER render, so the page briefly shows
     before redirecting. That's fine — it's near-instant. */
  useEffect(() => {
    if (user && !loading) {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    e.preventDefault(); // Prevent form from actually submitting (which would reload the page)
    setError("");
    setIsSubmitting(true);

    try {
      await signInWithEmail(email, password);
      // AuthProvider will detect the user change and update state
      // Our useEffect above will then redirect
    } catch (err: unknown) {
